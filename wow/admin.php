    ...
    if ($_SERVER['REQUEST_METHOD'] == "POST")
    {
        ...
        if (isset($_POST['new_title']) && isset($_POST['new_body']))
        {
            ...
            if ($title != "" && $body != "")
            {
                ...
                if ($db->ExecuteStmt(Statements::INSERT_WOW_NEWS, $db->BuildStmtArray("issss", $user->GetId(), $user->GetUsername(), $title, $body, date("Y-m-d H:i:s"))))
                    $error = false;
            }
        }
        ...
        {
            $db = new Database($DATABASES['USERS']);
            if ($db->ExecuteStmt(Statements::DELETE_WOW_NEWS, $db->BuildStmtArray("i", $_POST["new_delete"])))
                $error = false;
        }
        if (isset($_POST['from']))
        {
