...
if ($isAdmin)
{
    if ($_SERVER['REQUEST_METHOD'] == "POST")
    {
        ...
        if (isset($_POST['new_title']) && isset($_POST['new_body']))
        {
            ...
            $body = preg_replace("#(http://(www.)?youtube.com)?/(v/|watch\?v=)([a-z0-9\-_~]+)#i", '<div style="text-align:center;"><iframe width="640" height="480" src="http://www.youtube.com/embed/$4?wmode=transparent" frameborder="0" allowfullscreen></iframe></div><br />', $body);
        
            // Insert new in the database
            if ($title != "" && $body != "")
            {
                $db = new Database($DATABASES['USERS']);
                if ($db->ExecuteStmt(Statements::INSERT_WAR_THUNDER_NEWS, $db->BuildStmtArray("issss", $user->GetId(), $user->GetUsername(), $title, $body, date("Y-m-d H:i:s"))))
                    $error = false;
            }
        }
        ...
        {
            $db = new Database($DATABASES['USERS']);
            if ($db->ExecuteStmt(Statements::DELETE_WAR_THUNDER_NEWS, $db->BuildStmtArray("i", $_POST["new_delete"])))
                $error = false;
        }
        if (isset($_POST['from']))
        {
